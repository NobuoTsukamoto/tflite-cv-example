...
int main(int argc, char *argv[])
{
  ...
  // Window setting
  auto window_name = "Edge TPU Segmantation camouflage cpp demo.";
  ...
  while(cap.isOpened())
  {
    ...
    else
    {
      ...
      cv::Mat randam_im = cv::Mat::zeros(cv::Size(input_tensor_shape[1], input_tensor_shape[2]), CV_8UC4);

      RandamMaskImage(result, label_index, input_im, rng, randam_im, label_mask);

      // output tensor size => camera resolution
      cv::resize(randam_im, randam_im, cv::Size(frame.cols, frame.rows));
