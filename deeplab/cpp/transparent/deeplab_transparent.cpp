...
int main(int argc, char *argv[])
{
  ...
  const cv::String keys =
        ...
        "{l label  |15  | index of the target label for transparent. (default: Person)} "
  ...
  // Window setting
  auto window_name = "Edge TPU Segmantation transparent cpp demo.";
  ...
  // video capture.
  // First, detects images that do not show the target.
  std::vector<double> inference_fps;
  std::vector<double> fps;
  auto is_first_step = true;
  cv::Mat background_im;
  ...
  while(cap.isOpened())
  {
    ...
    cv::Mat frame, input_im, output_im;
    ...
    auto is_in_label = IsLabelInResult(result, label_index, input_tensor_shape[1], input_tensor_shape[2]);
    if (!is_in_label)
    {
      // Update background image.
      background_im = frame.clone();
    }
    ...
    {
      cv::Mat label_mask = cv::Mat::zeros(cv::Size(input_tensor_shape[1], input_tensor_shape[2]), CV_8UC3);

      LabelMaskColorImage(result, label_index, input_im, label_mask);

      // output tensor size => camera resolution
      cv::resize(label_mask, label_mask, cv::Size(frame.cols, frame.rows));

      cv::addWeighted(background_im, 1.0, label_mask, 0.5, 0, output_im);
    }
    ...
    // Display image.
    cv::Mat display_im;
    cv::hconcat(output_im, frame, display_im);
    cv::imshow(window_name, display_im);
